...
from lib.hyper.configs.model_configs import MPNNPOMConfig
...
class CV:
    ...
    def cross_validation(
        ...
        for fold_num, (train_dataset, valid_dataset) in enumerate(self.folds_list):
            logger.info("Fitting model %d/%d folds" % (fold_num + 1, self.n_folds))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--logdir", default="./models", help="Log directory to store results"
    )
    parser.add_argument(
        "-e",
        "--max_epoch",
