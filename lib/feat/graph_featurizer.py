...
from lib.utils.molecule_feature_utils import (
    get_atomic_num_one_hot,
    get_atom_total_valence_one_hot,
)
import logging
...
class GraphConvConstants(object):
    ...
    ATOM_FDIM = (
        sum(len(choices) + 1 for choices in ATOM_FEATURES.values())
        + len(ATOM_FEATURES_HYBRIDIZATION)
        + 1
    )
...
def bond_features(bond: RDKitBond) -> Sequence[Union[bool, int, float]]:
    ...
    """
    if bond is None:
        b_features: Sequence[Union[bool, int, float]] = [1] + [0] * (
            GraphConvConstants.BOND_FDIM - 1
        )
...
class GraphFeaturizer(MolecularFeaturizer):
    ...
    def _featurize(self, datapoint: RDKitMol, **kwargs) -> GraphData:
        ...
        return GraphData(
            node_features=f_atoms, edge_index=edge_index, edge_features=f_bonds
        )
